	...
	{#if $EditingTiles}
		<div class="absolute left-2 top-1/2 flex -translate-y-1/2 items-center gap-2">
			<button
				...
				class=" flex items-center gap-2 rounded-md border border-zinc-700 bg-zinc-800 p-1 px-4 text-sm"
			>
		...
		<div class="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-2">
			<button
				...
				class={`flex items-center gap-2 rounded-md border p-1 px-4 text-sm ${$UnsavedChanges ? 'border-yellow-500 bg-yellow-600' : 'border-blue-500 bg-blue-600'}`}
			>
