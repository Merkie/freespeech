<script lang="ts">
	...
	import { navigateToBoardPage, parseBoardRoute } from '$ts/client/board-navigation';
	...
	function openHome() {
		resetBoardUi();
		const target = parseBoardRoute(homeHref);
		const current = parseBoardRoute(window.location.pathname);

		if (target && current) {
			navigateToBoardPage(target.projectId, target.pageId);
		} else if (target && !$NetworkOnline) {
	...
	function requestHome(event: MouseEvent) {
		...
		if (!$UnsavedChanges) {
			...
			$DiscardUnsavedChangesHandler = openHome;
...
>
	...
	<button
		...
		class={cn('flex-1 rounded-md p-1 text-center transition-colors', {
			'bg-zinc-800': !$page.url.pathname.startsWith('/app/dashboard') && $EditingTiles,
			'pointer-events-none opacity-50': !$NetworkOnline && !$EditingTiles
		})}
		disabled={$page.url.pathname.startsWith('/app/dashboard') ||
			(!$NetworkOnline && !$EditingTiles)}
		title={$EditingTiles
			? 'Exit edit mode'
			: $NetworkOnline
				? 'Edit board'
				: 'Editing is unavailable offline'}><i class="bi bi-pencil-fill"></i></button
	>

	<!-- Dashboard Button -->
	<button
		on:click={requestDashboard}
		disabled={$EditingTiles || !$NetworkOnline}
		title={$NetworkOnline ? 'Open dashboard' : 'Dashboard is unavailable offline'}
		class={cn('flex-1 rounded-md p-1 text-center transition-colors', {
			'bg-zinc-800': $page.url.pathname.startsWith('/app/dashboard'),
			'pointer-events-none opacity-50': $EditingTiles || !$NetworkOnline
