...
def createindex():
    ...
    <center><h1>Freespeech <small>Alpha</small></h1></center>
    <center><strong><a onclick="editMode();">Edit mode</a></strong></center>
    <div id="content">
    '''
    ...
    def create_page(page, start):
        ...
        for row in start:
            ...
            for box in start[row]:
                if str(start[row][box])[0] == "#":
                    built += '<div class="box" style="background-color: '+start[row][box]+'"><a onclick="eel.speak(this.innerHTML);"><div class="inner"><p>'+box+'</p></div></a></div>'
                else:
                    if str(start[row][box]) == "home":
                        built += '<div class="box" style="border-radius: 25px;"><a href="index.html" style="color:white"><div class="inner"><p><i>Home</i></p></div></a></div>'
                    else:
                        newpage = open(str(count)+".html", "w+")
                        create_page(newpage, start[row][box]["page"])
                        built += '<div class="box" style="border-radius: 25px;"><a href="'+str(count)+'.html" style="color:white"><div class="inner"><p><i>'+box+'</i></p></div></a></div>'
                        count+=1
            built += '</div>'

        page.write(top+built+bottom)

    create_page(indexhtml, jsonfile["page"])
