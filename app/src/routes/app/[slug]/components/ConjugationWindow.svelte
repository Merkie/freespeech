<script lang="ts">
	// Stores
	import { ConjugatingTile, AppProject, CurrentPageId } from '$lib/client/stores';
	...
	const get_conjugations = async () => {
		...
		// filter current word
		conjugations = conjugations.filter(
			(conjugation) => conjugation.toLowerCase() !== current_word.toLowerCase()
		);
		// only allow letters in every word
		conjugations = conjugations.filter((conjugation) => conjugation.match(/^[a-zA-Z]+$/));
		// filter dupelicates
		conjugations = conjugations.filter(
			(conjugation, index) => conjugations.indexOf(conjugation) === index
		);
		// only take first 5
		...
		$AppProject.pages
			...
			.tiles.find((tile) => tile.id === $ConjugatingTile?.id).conjugations = conjugations;
