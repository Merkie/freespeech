<script lang="ts">
	...
	onMount(() => {
		...
		cy.$('node').on('tap', function (e) {
			var ele = e.target;
			const new_index = $ProjectData.pages.findIndex(
				(page) => page.name.toUpperCase() === ele.id().toUpperCase()
			);
	...
	const handle_remove_page = async (name: string) => {
		...
		// Remove the page
		$ProjectData.pages = $ProjectData.pages.filter((page) => page.name !== name) as TilePage[] &
			{ tiles: Tile[] }[];
...
<h1>Settings</h1>
<button on:click={() => window.location.assign('/dashboard')} class="dashboard-btn"
...
<section class="vitals">
	...
	<p>
		...
		<select
			on:change={(e) => ($UserSentenceBuilder = e.target.value == 'true')}
			value={$UserSentenceBuilder ? 'true' : 'false'}
		>
