<script lang="ts">
	...
	const navigate = async (navigation: string) => {
		// get new Index
		let newIndex = $ProjectData.pages.findIndex(
			(page) => page.name.toUpperCase() === navigation.toUpperCase()
		);
		...
		if (newIndex !== -1) {
			...
			response.page.columns = $ProjectData.pages[0].columns;
	...
	const handleInteraction = async () => {
		...
		if ($IsInEditMode) {
			...
			if ($IsEditingTrash) {
				$ProjectData.pages[$CurrentPageIndex].tiles = $ProjectData.pages[
					$CurrentPageIndex
			...
			if ($IsEditingTemplate) {
				if (tile.link) {
					...
					let parentPage = $ProjectData.pages.find(
						(page) => page.name.toUpperCase() === $PageHistory[1].toUpperCase()
					);
					if (parentPage) {
						let parentTile = (parentPage as TilePage & { tiles: Tile[] }).tiles.find(
							(parentTile) => parentTile.index === tile.index
						);
						if (parentTile) {
		...
		} else if (tile.navigation) {
			// If the user is not in edit mode and the tile has a navigation
...
<button
	disabled={loading}
	style={`background: ${tile.backgroundColor || 'auto'};
								...
								opacity: ${tile.silent && $IsEditingSilent && $IsInEditMode ? 0.5 : 'auto'};
...
>
	<input
		...
		bind:this={fileInput}
		bind:files
		on:change={async () => await handle_upload(files[0])}
		type="file"
	/>
	...
	{#if tile.navigation}
		<div
			class="folder-piece"
			style={`background: ${tile.backgroundColor || 'auto'}; border-color: ${
				tile.borderColor || 'auto'
			}; opacity: ${tile.invisible ? 0 : 1}`}
		/>
	{/if}
	...
	{#if tile.accented}
		<div
			class="corner-piece"
			style={`background: ${tile.borderColor || 'auto'}; opacity: ${tile.invisible ? 0 : 1}`}
		/>
	{/if}

	<p
		on:keyup={(e) => {
			tile[$EditingTextType] = e.originalTarget.innerText;
			add_to_edited_tiles();
		}}
		contenteditable={$EditingTextTile == tile.id && $EditingTextType === 'display'}
		style={'font-size: ' +
			(!tile.image && tile.display.length < 4 ? ($UserTileSize / 2).toString() + 'px' : 'inherit')}
	>
		{#if $EditingTextType === 'speak' && $IsEditingText && $IsInEditMode}
			<p
				contenteditable={$EditingTextTile == tile.id && $EditingTextType === 'speak'}
				style="all: unset; opacity: 0.5; font-size: small;"
			>
				{tile.speak || '...'}
			</p>
		{:else}
