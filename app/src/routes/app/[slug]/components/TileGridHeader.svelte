<script lang="ts">
	...
	const navigate_backwards = async () => {
		...
		$CurrentPageIndex = $ProjectData.pages.findIndex(
			(page) => page.name.toUpperCase() === $PageHistory[$PageHistoryIndex].toUpperCase()
		);
	};
...
<main>
	<div>
		<button
			...
			disabled={!($PageHistory.length > 1 && $PageHistoryIndex < $PageHistory.length - 1)}
			on:click={navigate_backwards}
		>
