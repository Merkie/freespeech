...
<main
	style={`background-color: ${
		!$InSettingsMenu ? 'var(--tile-grid-background)' : 'var(--background)'
	};`}
>
	{#if !$InSettingsMenu}
		<SentenceBuilder />
...
<AppNavigation>
	<AppNavigationButton
		...
		callback={() => {
			$CurrentPageIndex = $ProjectData.pages.findIndex(
				(page) => page.name.toUpperCase() === 'HOME'
			);
	...
	/>
	<AppNavigationButton name={'Settings'} iconsrc={Cog} callback={() => ($InSettingsMenu = true)} />
</AppNavigation>
